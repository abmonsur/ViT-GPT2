...
state_dict = torch.load("captioner/captioner.pt", map_location=device)
model.load_state_dict(state_dict, strict=False)
model.eval()
print(" Loaded custom VisionGPT2Model weights")

tokenizer = GPT2TokenizerFast.from_pretrained("gpt2")
tokenizer.pad_token = tokenizer.eos_token


base_path = Path("flickr30k/flickr30k-images")
df = pd.read_csv("flickr30k/flickr_annotations_30k.csv")

df["raw"] = df["raw"].map(ast.literal_eval)

test_df = df[df["split"] == "test"].reset_index(drop=True)

records = []
for _, row in test_df.iterrows():
    for caption in row["raw"]:
        records.append({
            "image": base_path / row["filename"].strip(),
            "caption": caption.strip().lower()
        })

test_df = pd.DataFrame(records)
print(f" Loaded {len(test_df)} image–caption pairs for evaluation")
...
def generate_caption(model, image_path, tokenizer, tfms, device, max_tokens=50):
    image = Image.open(image_path).convert("RGB")
    image = np.array(image)
    image = tfms(image=image)["image"].unsqueeze(0).to(device)
    sequence = torch.ones(1,1).long().to(device) * tokenizer.bos_token_id
    caption_ids = model.generate(image, sequence, max_tokens=max_tokens, temperature=1.0, deterministic=True)
    return tokenizer.decode(caption_ids.numpy(), skip_special_tokens=True)

preds, refs = [], []
for _, row in tqdm(test_df.iterrows(), total=len(test_df), desc="Generating captions"):
    pred = generate_caption(model, row["image"], tokenizer, tfms, device)
    preds.append(pred)
    refs.append([row["caption"]])

test_df["predicted"] = preds
test_df.to_csv("flickr30k_custom_predictions.csv", index=False)
